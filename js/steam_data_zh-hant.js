...
if (!steamDataElement) {
    throw new Error('頁面中缺少 Steam 資料卡資料。');
}

const data = JSON.parse(steamDataElement.textContent);
const formatNumber = new Intl.NumberFormat('zh-TW', { maximumFractionDigits: 1 });
...
document.getElementById('hours').textContent = `${formatNumber.format(data.summary.hours)} 小時`;
document.getElementById('average').textContent = `${formatNumber.format(data.summary.average)} 小時`;

if (data.updatedAt) {
    const updated = new Date(data.updatedAt);
    const time = document.getElementById('updated');
    time.dateTime = updated.toISOString();
    time.textContent = updated.toLocaleDateString('zh-TW', { year: 'numeric', month: 'short', day: 'numeric', timeZone: 'UTC' });
}
...
games.forEach((game) => {
    ...
    strong.textContent = game.hours == null ? '等待同步' : `${formatNumber.format(game.hours)} 小時`;
    playtime.append('遊玩時長：', strong);
